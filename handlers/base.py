...
@router.message(F.text == "? Помощь") # Или ваш фильтр/команда /help
async def cmd_help(message: Message):
    # 🧹 Чистим чат: удаляем пользовательское сообщение "? Помощь"
    try:
    ...
    help_text = (
        ...
        "📥 **Как правильно вносить расходы:**\n"
        "Отправьте в чат сообщение в формате: `Сумма Заметка`.\n\n"
        "⚠️ **ВАЖНО:** Вводите сумму **СТРОГО БЕЗ ПРОБЕЛОВ** в тысячах!\n"
        "❌ Неправильно: `1 153 мощный снюс` (бот запишет только 1 рубль)\n"
        "✅ Правильно: `1153 мощный снюсик`\n\n"
        "📊 **Кнопки меню:**\n"
        "• **Статистика** — покажет общие траты за месяц по категориям.\n"
        "• **Бюджет** — настройка вашего лимита на текущий месяц."
    )
